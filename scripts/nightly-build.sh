#!/bin/bash
# nightly-build.sh - ECUxPlot Nightly Build Management Script
# This script helps manage the nightly build system and provides utilities
...
trigger_nightly_build() {
    log_info "Triggering nightly build workflow..."

    check_gh_cli

    gh workflow run nightly-build.yml
    log_success "Nightly build workflow triggered"
...
# Check the status of the latest nightly build
check_build_status() {
    log_info "Checking latest nightly build status..."

    check_gh_cli

    local latest_run=$(gh run list --workflow=nightly-build.yml --limit=1 --json status,conclusion,createdAt,headSha --jq '.[0]')

    if [ "$latest_run" = "null" ]; then
        log_warning "No nightly builds found"
        return 1
    fi
    ...
    if [ "$status" = "completed" ] && [ "$conclusion" = "success" ]; then
        log_success "Latest nightly build completed successfully"
        return 0
    elif [ "$status" = "completed" ] && [ "$conclusion" = "failure" ]; then
        log_error "Latest nightly build failed"
        return 1
    else
        log_warning "Latest nightly build is still running"
        return 2
    fi
}
...
list_artifacts() {
    ...
    gh run list --workflow=nightly-build.yml --limit=5 --json databaseId,status,conclusion,createdAt,headSha --jq '.[] | select(.status == "completed" and .conclusion == "success") | {id: .databaseId, created: .createdAt, sha: .headSha}' | while read -r run_info; do
        if [ -n "$run_info" ]; then
...
# Show help
show_help() {
    echo "ECUxPlot Nightly Build Management Script"
    echo ""
    echo "Usage: $0 <command> [options]"
    echo ""
    echo "Commands:"
    echo "  trigger              Trigger a manual nightly build"
    echo "  status               Check the status of the latest nightly build"
